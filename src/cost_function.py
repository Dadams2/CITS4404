...
def costfn(clew: list[Camo_Worm], worm_idx: int, 
    ...
    for i in range(0, len(clew)):
        if i is not worm_idx:
            ...
            distance_score += min((1/eu_distance) if eu_distance != 0 else 1, 1)
    ...
    # Normalising intensity score (between 0 and 1)
    ...
    segment_score = 0
    if val0 != val1: segment_score += 0.3
    if val0 != val2: segment_score += 0.3
    if val1 != val2: segment_score += 0.3
    ...
    final_internal_score = internal_score * w_internal
    final_group_score = distance_score * w_dist
    final_env_score = environment_score * w_env

    final_score = final_internal_score + final_group_score + final_env_score

    return final_score, final_internal_score, final_group_score, final_env_score
